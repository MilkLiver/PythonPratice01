...
fro_m = 'D:/PyAI/OffsetCarXml/'#來源XML
t_o = 'D:/PyAI/NewOffsetCarXml/'#目的XML

fro_m2 = 'D:/PyAI/NewCarPhotos/'#影像來源資料夾
t_o2 = 'D:/PyAI/Newcars_train/'#影像目的資料夾

m=[90,180,270]  #調整角度(向右旋轉)
...
def func(file,i,A):
    global sum2,sum3,sum4,sum5,sum6
...
#start=time.time()
for i in m:
    tpool=ThreadPool(processes=threadNum)
    A = '_' + r + str(i)
    if (i == 0):
        A=''
    sum1,sum2 = 0,0
    sum4,sum5 = 0,0
    for file in os.listdir(fro_m)[:100]:  #來源XML
        tpool.apply_async(func,args=(file,i,A,))
        sum1 +=1
    tpool.close()
    tpool.join()
